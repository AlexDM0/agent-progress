...
| `skill/Reference.md` | What the help does not print, read only when it is needed: the ticket file format, the transition table, how the hook adds to a row's tokens, the concurrency limit, what `agent-progress release` does and its refusal reasons, the time axis and the exit codes. |
