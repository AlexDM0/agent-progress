...
| `skill/Reference.md` | What the help does not print, read only when it is needed: the ticket file format, the transition table, where a review row is drawn, how the hook adds to a row's tokens, the concurrency limit, the dispatcher state and the run id stored beside it, what `agent-progress release` does — the running review rows it delivers included — and its refusal reasons, the time axis and the exit codes. |
