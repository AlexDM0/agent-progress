...
| `cli/ticket/TicketCommand.ts` | `ticket add\|list\|show\|start\|review\|rereview\|done\|deliver\|abandon\|reopen\|status\|link`: the markdown tickets and the rows they drive. The named verbs enforce the legality matrix and `ticket status` is the override; `rereview` is the one verb legal on the status the ticket already holds; `show` always prints the file path |
