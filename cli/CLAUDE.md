...
| `cli/ticket/TicketCommand.ts` | `ticket add\|list\|show\|start\|claim\|review\|rereview\|done\|deliver\|abandon\|reopen\|status\|link\|depends\|priority`: the markdown tickets and the rows they drive. The named verbs enforce the legality matrix and `ticket status` is the override; `rereview` is the one verb legal on the status the ticket already holds; `show` always prints the file path. `claim` is `start` plus the row's `--owner` and `--note`, and **refuses** where `start` only warns: an unsettled dependency, a low ticket while a normal or high one is neither delivered nor abandoned, or running rows already at the concurrency limit — every check inside the one `openTrackerForWriting` callback, so two racing claims cannot both pass the count. Every `--json` ticket spells out its `priority`, `normal` where the file has none |
