...
| `cli/dispatcher/DispatcherCommand.ts` | `dispatcher [running\|finished\|stopped]`: print the stored state without a lock (`finished` on a tracker that never set one, writing nothing), or store one under the lock with one log line. An unknown word is refused at exit 1 before anything is written |
| `cli/dispatcher/DispatcherCommand.spec.ts` | a file without the field reading `finished` and left byte-identical, each state stored and read back, the refusals leaving the file byte-identical, a malformed stored state at exit 2, `status --json` carrying it, and the three advisories ending `status`'s Next line |
