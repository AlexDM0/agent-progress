...
| `cli/dispatcher/DispatcherCommand.ts` | `dispatcher [running\|finished\|stopped]`: print the stored state without a lock (`stopped` on a tracker that never set one, writing nothing), or store one under the lock with one log line. An unknown word is refused at exit 1 before anything is written |
