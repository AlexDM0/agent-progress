...
`init`, `task`, `ticket`, `log`, `concurrency`, `range`, `release` and `clear` all follow one sequence, and the order of its
...
| `cli/status/StatusCommand.ts` | `status`: the working view — counts, the rows and tickets that are not delivered or abandoned, the recent log ordered by its stamps, and under `--json` an `omitted` count of what was left out. `--full` is the *whole progress file plus every ticket's frontmatter*. Both `--json` documents carry the derived `concurrency` block: `limit`, `agentsInFlight`, `freeSlots` and `readyTicketIds` (in dispatch order, high first); every ticket in them spells out its `priority`; the human output ends with the Next line built from it |
...
| `cli/concurrency/ConcurrencyCommand.ts` | `concurrency [<n>]`: print the limit the board holds (2 on a tracker that never set one), or store a new whole number of at least 1 under the lock with one log line. A limit below the agents in flight is accepted |
