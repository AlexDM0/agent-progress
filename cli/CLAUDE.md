...
| `cli/CommandSupport.ts` | the sequence every mutating command follows, `resolveAtOption`, `printEntity`, `renderDashboard`, and `progressOperations` — the progress store in the shape `lib/tickets/TicketTransitions.ts` asks for. Also `concurrencyDocumentOf` (the one place readiness and the dispatcher state are read, for `status --json` and the Next line), `nextLineFor` (which adds the ready ids that are low priority, for the Next line's triage advice), `openTrackerForWritingThenReadNextLine` — the same sequence, reading the Next line from the files just written, inside the lock — and `printEntityThenNextLine`, which drops the line under `--json` |
| `cli/NextLine.spec.ts` | the Next line ending the human output of `status`, `ticket add\|claim\|review\|deliver\|abandon` and `task finish\|deliver`, table-driven with the dispatcher running so no advice is appended, each describing the board after the move; and none of their `--json` documents carrying it |
...
| `cli/update/UpdateCommand.spec.ts` | each item's `unchanged`/`updated` verdict against a file that really differs, the hook in the local settings file by default and refreshed in place in the shared one, the workflow unchanged on a second run and restored after a hand edit, `--no-claude-md`, `--no-hooks` and `--no-workflow`, the progress file and tickets left byte-identical, the worktree, and the refusals |
| `cli/status/StatusCommand.ts` | `status`: the working view — counts, the rows and tickets that are not delivered or abandoned, the recent log ordered by its stamps, and under `--json` an `omitted` count of what was left out. `--full` is the *whole progress file plus every ticket's frontmatter*. Both `--json` documents carry the derived `concurrency` block: `limit`, `agentsInFlight`, `freeSlots`, `readyTicketIds` (in dispatch order, high first) and `dispatcherState`, plus `dispatcherRunId` while one is stored; every ticket in them spells out its `priority`; the human output ends with the Next line built from it |
| `cli/status/StatusCommand.spec.ts` | both `--json` documents' shapes, which are the contract with the orchestrating agent, the concurrency block and the run id it carries only while one is stored, plus the token column and the log's order |
...
| `cli/ticket/TicketCommand.ts` | `ticket add\|list\|show\|start\|claim\|review\|rereview\|done\|deliver\|abandon\|reopen\|status\|link\|depends\|priority`: the markdown tickets and the rows they drive. The named verbs enforce the legality matrix and `ticket status` is the override; `rereview` is the one verb legal on the status the ticket already holds; `show` always prints the file path. `claim <id> [<id>...]` is `start` plus the row's `--owner` and `--note` for every ticket named, as **one agent**: each row gets the same `agent` key (the claimed ids joined), and it **refuses** where `start` only warns: an unsettled dependency outside the claim (one on a ticket in the same claim counts as settled), a low ticket while a normal or high one is neither delivered nor abandoned, or agents in flight already at the concurrency limit — every check and every move inside the one `openTrackerForWriting` callback, so a bundle is all or nothing and two racing claims cannot both pass the count. `--json` prints the ticket for one id and the list for several. Every `--json` ticket spells out its `priority`, `normal` where the file has none |
