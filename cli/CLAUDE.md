...
| `cli/release/ReleaseCommand.ts` | `release <id> [<id>...] --branch`: inside the one `openTrackerForWritingThenReadNextLine` hold — the lock is what serialises two releases — check each ticket is in-progress or in-review, the main checkout (the tracker root) is on `--main` and the branch descends from it, `git merge --ff-only` the checked commit, then move each ticket done and delivered through `applyTicketTransition`. Every refusal is thrown before the merge and leaves main and the tracker as they were; `main-moved` is the one a reviewer acts on. The worktree removal and `branch -d` run after the lock and are reported, never failed, at exit 0. The human output ends with the Next line, read after the delivery inside the lock hold; `--json` prints `{released: true, tickets, branch, mainLine, commit, cleanup}` on success and `{released: false, reason, detail, cleanup: []}` on every refusal, and carries no Next line |
| `cli/release/ReleaseCommand.spec.ts` | a scratch repository with the tracker in its main checkout and the work on linked worktrees: the fast-forward with the ticket delivered and worktree and branch gone, a bundle, a run from inside the worktree, an untracked file left standing at exit 0, each refusal leaving main, both tracker files and the worktree unchanged, two concurrent releases never both merging, the `--json` key sets and the four cleanup step shapes read from `cli/HelpText.ts` and `skill/Reference.md` and held to what is printed, and the Next line ending the human output — agreeing with `status --json` afterwards — absent from `--json` |
