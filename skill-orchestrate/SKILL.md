...
flight holds. Each ticket keeps its own row: `ticket start` every one of them, and when the agent
lands, `ticket review` each, dividing the agent's `--tokens` evenly over the bundle with
`--note "bundle #a #b #c"`. The agent commits and hands off per ticket, so a bundle is still judged,
