...
| file | what it is |
|---|---|
| `skill-orchestrate/SKILL.md` | The skill itself: the opening sequence, how a ticket request is grilled and filed, the two-slot dispatch loop, what happens when a result lands, what the orchestrator keeps in its own working memory and what it drops, and the board hygiene that keeps the chart true. |
