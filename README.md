...
- **`.agent-progress/`** at the repository root, holding `progress.json`, a `tickets/` folder, the
  generated `progress.html` and the `.lock` directory. The root is found with
...
A seventh is the worker agent definition, **`.claude/agents/agent-progress-worker.md`**, a Claude Code
...
- **A pill naming the state the row is actually in, where `done` means merged.** The stored status
  ...
  then how many are awaiting a merge and how many are in review. A row with nothing to merge — a
  review pass, a chore — still reaches `done`, through `agent-progress task deliver <id>`; a dispatched reviewer
...
| `release <id> [<id>...] --branch <b> [--worktree <path>] [--main <line>] [--json]` | Release a reviewed branch, and the only way one reaches the main line: allowing this command in the harness is the release permission, and nobody runs `git merge` into main by hand. In one lock hold, so two releases never race, it checks that each ticket is in-progress or in-review, that the main checkout — the tracker's root, found the same way from any worktree — is on `--main` (default `main`) and that `<b>` descends from it, fast-forwards, and moves each ticket done and delivered with `--branch <b>` and `--commit` set to the merged tip. In the same hold every `running` review row whose `--review-of` names a released ticket is finished and delivered at the release time, because the reviewer releases as the last step of its pass; a row linked only by its name is left alone, and the output names each row it closed. Several ids are the tickets of one bundle on one branch. Every refusal changes nothing, the review rows included. Most reasons (`invalid-request`, `unknown-ticket`, `ticket-not-releasable`, `unknown-branch`, `not-on-main-line`, `main-moved`, `merge-refused`) exit 1; the one to act on is `main-moved`: rebase `<b>` onto the main line, re-run the checks, count the rebase with `rework --rebased-from` and release again. `git-failed` and `tracker-failed` — git or the tracker could not be read — exit 2. Afterwards it runs `git worktree remove` on `--worktree`, never forced, and `git branch -d <b>`; what git declines — a worktree holding untracked files, say — is named with its files at exit 0, since the release happened. `--json` prints `{released: true, tickets, branch, mainLine, commit, closedReviewRows, cleanup}` on success — `closedReviewRows` the ids of the review rows it delivered — and `{released: false, reason, detail, cleanup: []}` on a refusal; `agent-progress help` gives each cleanup step's shape. |
| `ticket add "<title>" [--type bug\|change\|feature] [--priority low\|normal\|high] [--model <m>] [--effort <e>] [--group <name>] [--depends-on <ids>] [--body <markdown>] [--body-file <path\|->] [--at <when>]` | File a ticket: a markdown file under `.agent-progress/tickets/` with its own frontmatter, plus a pending Gantt row unless it is low. `--priority` defaults to normal; a low ticket is filed with no row and takes no task id until it is started. The body comes from the template, from `--body`, or from `--body-file` (`-` reads standard input); afterwards it is preserved byte for byte. `--depends-on 3,4` files it already waiting on those tickets. `--model` (haiku, sonnet, opus, fable) and `--effort` (low, medium, high, xhigh, max) name what its building and reviewing agents run on; left off, they are opus and medium, and anything else is refused at exit 1. |
| `ticket list [--status <s>] [--priority <p>] [--json]` | The tickets with their status, priority, type and row id, and the model and effort after the title where the ticket names them. `--status` and `--priority` narrow the listing. `--json` carries no bodies; use `ticket show` for one ticket's prose. |
| `ticket show <id> [--json]` | One ticket: its frontmatter, its model and effort where it names them, its body, and always its file path. |
| `ticket agent <id> [--model <m>] [--effort <e>] [--at <when>]` | Change the model or the effort a ticket's agents run on, or both, with one log line such as `Ticket #003 agents opus/medium → sonnet/medium`. Refused at exit 1, writing nothing, on a delivered or abandoned ticket, with neither option, with a value outside the two lists, and when the resolved pair would not change. |
| `ticket hold <id> [--reason <text>] [--at <when>]` / `ticket unhold <id> [--at <when>]` | Pause a ticket between build and review, or between review rounds, without stopping the dispatcher: while the frontmatter's `hold` key is set (to the reason, empty without one) the run starts no builder or reviewer for the ticket and releases a row it left running for it, `ticket claim` refuses it, and `status --json` lists it in `concurrency.heldTicketIds` and marks its `readyTickets` entry `held: true`. Once unheld, the pending step starts at the run's next board read; a run that ends first returns it under `held`. An agent already running is never interrupted: a hold set after a builder's final status read is too late for the reviewer it starts. One log line each way; refused at exit 1, writing nothing, on a delivered or abandoned ticket, and when the ticket already is or is not held. |
...
| `ticket start\|review\|done\|deliver\|abandon\|reopen <id> [--branch <b>] [--commit <sha>] [--reason <text>] [--tokens <n>] [--at <when>]` | Move a ticket and its Gantt row together, stamping both, and set the row's token count. Each verb only moves a ticket that is in a status it makes sense from (see the table below), and a move to the status a ticket already has is refused. `abandon` requires `--reason`; `reopen` clears the stamps and returns the row to pending. `--tokens` on a ticket with no row (a low one never started) is refused at exit 1. |
| `ticket review\|rereview <id> --start-review [--owner <who>] [--note <text>] [--at <when>]` | The move to review, or the next round, and the reviewer's running bar (`Review <N> #<id> — <title>`, its `reviewOf` the ticket, N the `## Review` sections plus one) in one lock hold, closing any bar of the round before: the builder's slot passes to its reviewer, and one round's to the next, without `status --json` ever showing it free. `--owner` and `--note` name the bar, and are refused without the flag. |
| `ticket claim <id> [<id>...] [--owner <who>] [--note <text>] [--at <when>]` | `ticket start` and the row's `--owner` and `--note` in one write, for every ticket named, as one agent: a bundle's builder claims all its tickets in the one call, and their rows share one agent key. Refused at exit 1 with nothing written, all or nothing, when any ticket is not open or in-review, is held, waits on a ticket outside the claim that is not done or delivered (one inside it is settled), is low while a normal or high ticket is not yet delivered or abandoned, has a review bar running, or when the agents in flight already number the concurrency limit. The count and the moves share one lock hold, so two claims racing for the last slot cannot both succeed. `--json` prints the ticket, or with several ids the list of them. The first command an implementing agent runs. |
...
| `ticket status <id> <status> [...same options]` | The same move, naming the target status directly: open, in-progress, in-review, done, delivered or abandoned. It takes the same options, and is the documented way to make a move the verbs refuse. |
